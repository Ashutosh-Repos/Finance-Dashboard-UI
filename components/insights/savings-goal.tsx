...
export function SavingsGoal() {
  ...
  const { currentSavings, percentage, isOnTrack } = useMemo(() => {
    // Latest month with data (June 2026)
    const juneTxs = transactions.filter((t) => t.date.startsWith("2026-06"))
    const income = juneTxs.filter((t) => t.type === "income").reduce((s, t) => s + t.amount, 0)
    const expenses = juneTxs.filter((t) => t.type === "expense").reduce((s, t) => s + t.amount, 0)
    const savings = Math.max(0, income - expenses)
    const pct = Math.min((savings / SAVINGS_GOAL_MONTHLY) * 100, 100)
    return { currentSavings: savings, percentage: pct, isOnTrack: pct >= 80 }
  }, [transactions])
        ...
        <CardContent className="flex flex-col items-center gap-4">
          ...
          <div className="text-center">
            <p className="text-sm font-medium">
              {formatCurrency(currentSavings)} saved this month
            </p>
